...
#define BIT_ALT_STAT_DRDY 0x40  // 设备准备好
...
static void identify_disk(struct disk* hd) {
    ...
    char buf[64];
    uint8_t sn_start = 10 * 2, sn_len = 20;  // 序列号起始位置和长度
    uint8_t md_start = 27 * 2, md_len = 40;  // 固件版本号起始位置和长度
    swap_pairs_bytes(&id_info[sn_start], buf, sn_len);
    printk("    disk %s info:\n       SN: %s\n", hd->name, buf);
    memset(buf, 0, sizeof(buf));
    swap_pairs_bytes(&id_info[md_start], buf, md_len);
    printk("       MODULE: %s\n", buf);
    uint32_t sectors = *(uint32_t*)&id_info[60 * 2];  // 得到扇区数
    printk("       SECTORS: %d\n", sectors);
...
static bool partition_info(struct list_elem* pelem, int arg UNUSED) {
    ...
    // list_traversal的回调函数只有返回true时才会继续向下遍历元素
    return false;
}
