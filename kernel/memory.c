 ...
 * 一个页框大小的位图可以表示128MB内存, 位图地址安排在地址0xc009a000
 * 这样本系统最大支持4个页框的位图, 即512MB
 */
#define MEM_BITMAP_BASE 0xc009a000

// 0xc0000000是内核从虚拟地址3G起
// 0x100000跨过1MB内存，使虚拟地址在逻辑上连续
#define K_HEAP_START 0xc0100000  // 设置堆起始地址进行动态分配

#define PDE_IDX(addr) ((addr & 0xffc00000) >> 22)  // 获取虚拟地址前十位,这里是PDE的索引值
#define PTE_IDX(addr) ((addr & 0x003ff000) >> 12)  // 获取虚拟地址的中间十位,这里是PTE的索引值
...
static void page_table_add(void* _vaddr, void* _page_phyaddr) {
     ...
     */
    // 先在页目录内判断目录项的P位, 若为1表示该表已经存在
    if (*pde & 0x00000001) {
        // 页目录项和页表项的第0位为p,这里是判断页目录项是否存在
        ASSERT(!(*pte & 0x00000001));  // 若是以前有已经装载的物理页框, 则会报错
        // 只要是创建页表, pte就不该存在, 多判断一下放心
        if (!(*pte & 0x00000001)) {
            *pte = (page_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
        } else {
            // 目前不会执行到此, 上面的ASSERT会先执行
            PANIC("pte repeat");
            *pte = (page_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
        }
    } else {
        // 页目录项不存在, 所以要先创建页目录再创建页目录项
        // 页表中用到的页框一律从内核空间分配
...
// 从内核物理内存池中申请pg_cnt个页内存, 成功返回虚拟地址, 失败返回NULL
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    // 以页为单位的内存分配策略, 不足1页不考虑
    uint16_t all_free_pages = free_mem / PG_SIZE;
    uint16_t kernel_free_pages = all_free_pages / 2;
    uint16_t user_free_pages = all_free_pages - kernel_free_pages;
    // 为简化位图操作, 余数不处理, 坏处是丢内存, 好处是不用做越界检查, 因为位图表示的内存小于实际物理内存
    uint32_t kbm_length = kernel_free_pages / 8;                 // kernel bitmap长度, 位图中1位表示1页, 以字节为单位
...
void* sys_malloc(uint32_t size) {
    ...
    if (size > 1024) {
        ...
        // 若申请的内存小于等于1024, 可以将其容纳在各种规格的mem_block中
        uint8_t desc_idx;
        // 从内存块描述符中匹配合适的内存块规格
        for (desc_idx = 0; desc_idx < DESC_CNT; desc_idx++) {
