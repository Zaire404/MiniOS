...
struct file file_table[MAX_FILE_OPEN];  // 全局文件表
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
    ...
    // 第一次写入时,如果是写入的数据比一个块的大小还大,
    // 则先将前面BLOCK_SIZE个字节写入到文件,剩下的再写入
    ...
    inode_sync(cur_part, file->fd_inode, io_buf);  // 同步inode信息到硬盘
    sys_free(all_blocks);
    sys_free(io_buf);
    return bytes_written;
}

// 从文件file中读取count个字节写入buf,成功则返回读出的字节数,到文件尾则返回-1
int32_t file_read(struct file* file, void* buf, uint32_t count) {
    uint8_t* buf_dst = (uint8_t*)buf;
    uint32_t size = count, size_left = size;

    // 若请求的字节数超过了文件可读的剩余量,就用剩余量作为待读取的字节数
