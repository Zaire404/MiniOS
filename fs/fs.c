...
static bool mount_partition(struct list_elem* pelem, int arg) {
    ...
    if (!strcmp(part->name, part_name)) {  // 如果找到了名为part_name的分区
        cur_part = part;
...
static void partition_format(struct partition* part) {
    ...
    printk(
        ...
        sb.magic, sb.part_lba_base, sb.sec_cnt, sb.inode_cnt, sb.block_bitmap_lba, sb.block_bitmap_sects, sb.inode_bitmap_lba, sb.inode_bitmap_sects,
    ...
    // 由于inode_table中共4096个inode, 位图inode_bitmap正好占用1扇区, 所以位图中的位全都代表inode_table中的inode
...
static int32_t search_file(const char* pathname, struct path_search_record* searched_record) {
    ...
    // 执行到此, 必然是遍历了完整路径并且查找的文件或目录只有最后一级
...
int32_t sys_mkdir(const char* pathname) {
    ...
    struct dir* parent_dir = searched_record.parent_dir;
    // 目录名称后可能会有字符'/', 所以最好直接用searched_record.searched_path,无'/'的路径做为参数
...
// 读取目录dir的下一目录项, 成功后返回其目录项地址, 到达目录尾则返回NULL
...
char* sys_getcwd(char* buf, uint32_t size) {
    ...
    // 至此full_path_reverse中的路径是反着的, 即子目录在前(左), 父目录在后(右)
    ...
    while (channel_no < channel_cnt) {
        ...
        while (dev_no < 2) {
            ...
            while (part_idx < 12) {   // 4个主分区+8个逻辑
                ...
                // 若partition未初始化,则partition中的成员默认为0,此时partition未初始化
                if (part->sec_cnt != 0) {  // 如果分区存在
                    memset(sb_buf, 0, SECTOR_SIZE);
                    // 读出分区的超级块,根据魔数是否正确来判断是否存在文件系统
                    ide_read(hd, part->start_lba + 1, sb_buf, 1);
                    // 只支持自己的文件系统.若磁盘上已经有文件系统就不再格式化了
                    if (sb_buf->magic == 0x20030804) {
